...
class ApplicationInsightsDataClientTests(ScenarioTest):
    ...
    def test_query_execute(self):
        ...
        self.cmd('az monitor app-insights query --apps 00000000-12e9-4631-bc02-50b965da2633 f4963800-c77d-40a3-8b0b-448678904c33 --analytics-query "availabilityResults | distinct name | order by name asc"', checks=[
            self.check('tables[0].rows[-1][0]', 'microsoft'),
            self.check('tables[0].rows[-2][0]', 'google')
        ])
        query_guid = self.cmd('az monitor app-insights query --app 00000000-12e9-4631-bc02-50b965da2633 --analytics-query "requests | getschema"').get_output_in_json()
    ...
    def test_metrics_show(self):
        ...
        azure_result = self.cmd('az monitor app-insights metrics show --app 00000000-12e9-4631-bc02-50b965da2633 --metric availabilityResults/count').get_output_in_json()
    ...
    def test_metrics_get_metadata(self):
        self.cmd('az monitor app-insights metrics get-metadata --app 00000000-12e9-4631-bc02-50b965da2633', checks=[
    ...
    def test_events_show(self):
        self.cmd('az monitor app-insights events show --app 00000000-12e9-4631-bc02-50b965da2633 --type availabilityResults --event 1f492e8f-de80-11e8-8fec-e18ec74f57af', checks=[
            self.check('value[0].ai.appId', '00000000-12e9-4631-bc02-50b965da2633'),
            self.check('value[0].availabilityResult.duration', 104),
            self.check('value[0].client.city', 'Boydton')
        ])

    def test_events_list(self):
        self.cmd('az monitor app-insights events list --timespan PT5M --app 00000000-12e9-4631-bc02-50b965da2633 --type availabilityResults', checks=[
            self.check('value[0].ai.appId', '00000000-12e9-4631-bc02-50b965da2633'),
        ])
        result = self.cmd('az monitor app-insights events list --timespan PT5M --app 00000000-12e9-4631-bc02-50b965da2633 --type availabilityResults').get_output_in_json()
